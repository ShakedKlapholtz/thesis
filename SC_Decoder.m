function [Estimated_U,Estimated_X,Estimated_L] = SC_Decoder(Lambda,U,N,Is_Frozen_Bit_Index_Vec,Is_Genie_Aided)
    ...
    if(N==2)
        ...
        [Estimated_U(1,1:next_N),Estimated_X(1:2:end),Estimated_L(1:next_N)] = SC_Decoder(L,U(1:next_N),next_N,Is_Frozen_Bit_Index_Vec(1:next_N),Is_Genie_Aided);       

        L = ((-1).^Estimated_X(1:2:end)).*temp_odd+temp_even;

        [Estimated_U(1,next_N+1:N),Estimated_X(2:2:end),Estimated_L(next_N+1:N)] = SC_Decoder(L,U(next_N+1:N),next_N,Is_Frozen_Bit_Index_Vec(next_N+1:N),Is_Genie_Aided);
