...
for SNR = 1:length(SNR_dB_range)
    ...
    frozen_bits_indicator = data1.frozen_indicator;
    ...
    for trial = 1:num_trials_per_snr
        ...
        %% ===== Decode =====
        if IS_SC
            [Estimated_U1,~,~] = SC_Decoder(Lambda1, u, N, frozen_bits_indicator, false);
            [Estimated_U2,~,~] = SC_Decoder(Lambda2, u, N, frozen_bits_indicator, false);
        else
            % AWGN
            [Estimated_U1,~] = gArikan_BPSK_SCL_Decoder(L,GA_CRC_Length,Y,u,Sigma,SNR_dB,frozen_bits_indicator,'AWGN');
            
            % BSC
            [Estimated_U2,~] = gArikan_BPSK_SCL_Decoder(L,GA_CRC_Length,Y,u,Sigma,SNR_dB,frozen_bits_indicator,'BSC');
        end
